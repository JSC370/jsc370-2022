...
Try these commands out (you may need to install them on your machine
first)
...
-   cross validation (in ML) `carat`, `mlr`
...
ans0 <- confint(lm(y~x))
ans1 <- my_boot(dat = data.frame(x=x, y=y), stat = my_stat,
                R = R, ncpus = 2L)

print(ans0)
print ( t(apply(ans1, 2, quantile, c(0.025, .975) )))
```
...
fun3 <- function(dat, stat, R){
  ...
  idx <- matrix(sample.int(n, n*R, replace = T), nrow=n, ncol = R)
